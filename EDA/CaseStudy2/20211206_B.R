...
timeSeriesPlot <- function (alldata, place, month, metric, yearRange){
  ...
  ggplot(data_subset)+labs(y = "", title="Please select the WEATHER MEASURE, the LOCATION and YEAR RANGE.\n The month is not used for this graph.",caption = "This data was gathered from the website https://www.metoffice.gov.uk/research/climate/maps-and-data/historic-station-data.")+
...
ui <- fluidPage(
  ...
  mainPanel(
    tabsetPanel(
      ...
      tabPanel("Weather Trend Each Year",
               plotOutput("plot")
...
server <- function(input, output, session) {
  ...
  #create Month comparison plot
  mmcomparisonplotgeneration <- reactive({
    mmcomparisonplot(alldata, input$place, input$investigationMetric)
  })
    ...
    output$map <- renderLeaflet({
      leaflet(options = leafletOptions(zoomControl = FALSE)) %>%
        ...
        addCircleMarkers(data = (filtered_data <- alldata %>%
          filter(yyyy==input$year_range[1], monthNamesShort==input$month)),
                         layerId=filtered_data$name,
                         color = ~pal(get(input$investigationMetric)),
                         group = "circles",
                         popup = paste(
                           "Station:", filtered_data$name, "<br/>",
                           "Month:", input$month, "<br/>",
                           "Year:", input$year_range[1],"<br/>",
    ...
    observe({
      ...
      output$timeSeries <- renderPlotly(
        plot_ly( data=(filtered_data2 <- alldata %>%
          filter(place==click$id,  monthNamesShort==input$month)),
  ...
  output$plot2 <- renderPlot({ mmcomparisonplotgeneration()  })
