...
ui <- fluidPage(
  ...
  sidebarPanel(
    ...
    # Define the inputs we want based on the user selection
    # Note the dynamic Shiny code was taken from: https://mastering-shiny.org/action-dynamic.html
    tabsetPanel(
      id = "params",
      type = "hidden",
      tabPanel("seasons",
               selectInput("year-from-seasons", "year from", choices = c()),
               selectInput("year-to-seasons", "years to", choices = c())
      ),
      tabPanel("location",
               selectInput("location-one", "Location 1", choices = c()),
               selectInput("location-two", "Location 2", choices = c())
      ),
      tabPanel("time",
               selectInput("yearFrom", "year from", choices = c()),
               selectInput("yearTo", "year to", choices = c())
      )
    )
  ),
  mainPanel(
...
server <- function(input, output) {
  ...
  # This is the key to how the parameters are displayed based on user input
  # what we are doing is watching for a change on the dropdown object and then
  # displaying the appropriate parameters as defined by the
  # object id "params" is used NOT the name - it doesn't have one!
  observeEvent(input$investigationType, {
    updateTabsetPanel(inputId = "params", selected = input$investigationType)
  })

    # update dropdowns with data required
  updateSelectInput(inputId = "year-from-seasons", choices = yearsOfInterest)
  updateSelectInput(inputId = "year-to-seasons", choices = yearsOfInterest)
  updateSelectInput(inputId = "yearFrom", choices = yearsOfInterest)
  updateSelectInput(inputId = "yearTo", choices = yearsOfInterest)
  updateSelectInput(inputId = "location-one", choices = weatherStationsOrdered)
  updateSelectInput(inputId = "location-two", choices = weatherStationsOrdered)
