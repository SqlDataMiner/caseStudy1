...
lungcancer.regexp <- survreg(lungcancer.sv ~ as.factor(q2$sex) + as.factor(q2$exer) + as.factor(q2$smok) + as.factor(q2$trea) + q2$age, dist="exponential")
...
S_b(t;\mathbf{x}) &= exp\{{-t\: exp(-0.5017 + 0.5328x_{sex} + 0.2566x_{exer} + 0.8821x_{smok} - 0.8144x_{trea} + 0.0113x_{age})}\}\\
\end{aligned}
$$

As $z = \beta_4 /(std. error) = 4.29$ and $4.29 > 1.96$ then there is overwhelming evidence that the treatment has an effect. Considering the proprtional hazards related to treatment A and B we know that $h(t; x |x_4=1)/h(t; x | x_4=0) = exp\:\beta_4$.  As $\beta_4 > 0$ then treatment B ($x_4 = 1$) increases the hazard relative to treatment A ($x_4 = 0$).
