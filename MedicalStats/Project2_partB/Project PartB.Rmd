...
$$
\begin{aligned}
h_a(t;\mathbf{x}) &= h_0(t)exp\{\beta_2x_2 + \beta_3x_3 + \beta_4x_4 + \beta_5x_5 \}\\
h_b(t;\mathbf{x}) &= h_0(t)exp\{\beta_1\times 1 + \beta_2x_2 + \beta_3x_3 + \beta_4x_4 + \beta_5x_5\}\\
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{h_b(t;\mathbf{x})}{h_a(t;\mathbf{x})}  &= \frac{h_0(t)exp\{\beta_1\times 1 + \beta_2x_2 + \beta_3x_3 + \beta_4x_4 + \beta_5x_5\}}{h_0(t)exp\{ \beta_1\times 0 + \beta_2x_2 + \beta_3x_3 + \beta_4x_4 + \beta_5x_5 \}}\\
...
# Question 2.3
The median is calculated as follows where $median_a$ and $median_b$ be the median survival times for treatments A and B respectively.:
$$
\begin{aligned}
\text{median}_a &= log(2)exp\{\beta_0 + \beta_1\times x_1 + \beta_2\times x_2 + \beta_3\times x_3 + \beta_4\times x_4 + \beta_5\times x_5\} \\
&= log(2)exp\{\beta_0 + \beta_1\times x_{treat} + \beta_2\times x_{sex} + \beta_3\times x_{exer} + \beta_4\times x_{smok} + \beta_5\times x_{age}\}\\
&= log(2)exp\{0.5017 + 0.8144\times 1 - 0.5328\times x_{sex} - 0.2566\times x_{exer} - 0.8821\times x_{smok} - 0.0113\times x_{age}\}
\\
\text{median}_b &= log(2)exp\{\beta_0 + \beta_2\times x_{sex} + \beta_3\times x_{exer} + \beta_4\times x_{smok} + \beta_5\times x_{age}\}\\
&= log(2)exp\{0.5017 - 0.5328\times x_{sex} - 0.2566\times x_{exer} - 0.8821\times x_{smok} - 0.0113\times x_{age}\}
\end{aligned}
$$
The question asks "using the R output" therefore we must tabulate the possible answers.  Age is the only continuous variable and is centered at fifty, meaning patients with age fifty will have an age of zero.  The following tabulation therefore will assume the age is fifty and thus not include it in the calculations thus permitting comparisions of treatments without generating too many permutations.  Note that given the short median times in years, it was considered useful to convert those to months.

Scenario (Age = 50, therefore $x_{age} = 0)     Median A (years)      Median B (years) Median A (months) Median B (months)
-------------------------------------------    ------------------     ----------------- ----------------- -----------------
...
******************************************************
TO CONSIDER Should we also add predecitions for age = 35 and age = 56?
****************************************************
# Question 2.4
