...
# Question 2.4
Considering the statement that the hazard ratios of females to males and smokers to non-smokers are the same.  We know that there are significant differences between treatment A and treatment B; therefore to avoid Simpson's paradox we must perform two hypotheses, namely:

* $\mathlarger{\frac{h_{female}(t;\mathbf{x}|\text{treatment A})}{h_{male}(t;\mathbf{x}|\text{treatment A})} = \frac{h_{smoker}(t;\mathbf{x}|\text{treatment A})}{h_{non-smoker}(t;\mathbf{x}|\text{treatment A})}}$
* $\mathlarger{\frac{h_{female}(t;\mathbf{x}|\text{treatment B})}{h_{male}(t;\mathbf{x}|\text{treatment B})} = \frac{h_{smoker}(t;\mathbf{x}|\text{treatment B})}{h_{non-smoker}(t;\mathbf{x}|\text{treatment B})}}$
